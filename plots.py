...
class Plotter:
    ...
    def bar_plot_with_points(self, df, chance_level, pvals=None, compare=False, hue="Modality",is_variance=False):
        ...
        if pvals is not None:
            for i, bar in enumerate(bplot.patches[:len(pvals)]):
                star = stars(pvals[i])
                if star != "ns":
                    bplot.annotate(star,
    ...
    def plot_accuracy_std_from_different_folders(self, folder_names, labels, title, hue, only_within=False):
        labels_ = {
            ...
            "cross": ["cross_V5_L", "cross_V5_R", "cross_PT_L", "cross_PT_R"],
        }
...
def interleave_lists(lists):
    ...
    for i in range(len(lists[0])):
        final_list.extend(lists[j][i] for j in range(len(lists)))
    return final_list
...
def plot_average_voxel_intensities(maps, classes, n_subjects):
    ...
    plt.title(f"Average voxel intensities for {region}")
    plt.legend()
    plt.savefig(f"plots/average_voxel_intensities{region}.png", dpi=600)
...
def plot_normalize_voxel_intensities(maps, classes):
    ...
    n_voxels = maps[0]["vis"][0][region].shape[1]
    mean_aud = {cla: np.zeros(n_voxels) for cla in classes}
    ...
    plt.xlabel("voxel id")
    plt.ylabel("intensity")
    plt.title(f"Original voxel intensities for audition in {region}")
    plt.legend()
    plt.savefig(f"plots/normalize_voxel_intensities{region}.png")
